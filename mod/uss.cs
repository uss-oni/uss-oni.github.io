...
namespace USS
{
  public class HopLeDlc
  {
    [HarmonyPatch(typeof(DlcManager))]
    [HarmonyPatch("IsDlcListValidForCurrentContent")]
    public class HopLeDlcManager {
  ...
  public class Patches
  {
    ...
    public class Db_Initialize_Patch
    {
      ...
      public static void ToRust(string dir, Dictionary<string, Dictionary<string, Dictionary<string, Entity>>> dico)
      {
        using var rustDb = new StreamWriter(Path.Combine(dir, "src", "db.rs"));
        using var rustTag = new StreamWriter(Path.Combine(dir, "src", "lang", "game.rs"));
        using var rustGame = new StreamWriter(Path.Combine(dir, "src", "lang", "temp", "game.rs"));
        using var rustDesc = new StreamWriter(Path.Combine(dir, "src", "lang", "temp", "desc.rs"));
        using var rustUi = new StreamWriter(Path.Combine(dir, "src", "lang", "temp", "ui.rs"));
...
""");
        rustTag.WriteLine("// Autogenerated");
        rustTag.WriteLine("#![allow(non_camel_case_types)]");
        rustTag.WriteLine("#[derive(Debug, Clone, Copy)]");
        rustTag.WriteLine("#[repr(i32)]");
        rustTag.WriteLine("pub enum Game {");
        rustGame.WriteLine("""
...
pub fn translate(game: Game) -> &'static str {
  match game {
""");
        rustUi.WriteLine("pub static UI: &[&str] = &[");

        HashSet<string> tagList = [];
        HashSet<string> locationList = [];
        HashSet<string> rotationList = [];
        HashSet<string> linksList = [];
        HashSet<string> modeList = [];
        foreach (var (category, subCategory) in dico)
        {
          foreach (var (sub, entities) in subCategory)
          {
            Dictionary<(string, Type), bool> list = [];

            // Retrieve all possibles fields
            foreach (var (_, entity) in entities)
            {
              foreach (var prop in entity.GetType().GetFields())
              {
                ...
                {
                  list[(prop.Name, prop.FieldType)] = true;
                }
              }
            }

            // Check if they exists in all entities
            foreach (var (_, entity) in entities)
            {
              foreach (var key in list.Keys.ToList())
              {
                var prop = entity.GetType().GetField(key.Item1);
                if (prop.GetValue(entity) == null)
                {
                  list[key] = false;
                }
            ...
            // Create the struct
            var structName = category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1);// sub.Split(new[] { "_" }, StringSplitOptions.RemoveEmptyEntries).Select(s => char.ToUpperInvariant(s[0]) + s.Substring(1, s.Length - 1)).Aggregate(string.Empty, (s1, s2) => s1 + s2);
            rustDb.WriteLine("pub struct " + structName + "{");
            foreach (var ((name, type), b) in list)
            {
              var typeName = "";
              switch (name)
              {
                ...
                case "material_category":
                  typeName = "&'static [MaterialCategory]";
                  break;
                case "molar_mass":
                  typeName = "crate::units::GramPerMole";
                  break;
                case "shc":
                  typeName = "crate::units::DtuPerGramPerDegree";
                  break;
                case "radiation_absorption_factor":
                case "flood_threshold":
                case "light_absorption":
                  typeName = "crate::units::Percent";
                  break;
                case "thermal_conductivity":
                  typeName = "crate::units::DtuPerMetreSecondPerDegree";
                  break;
                case "mass_per_unit":
                case "chunk_mass":
                case "mass":
                case "default_mass":
                  typeName = "crate::units::Kg";
                  break;
                case "height":
                  typeName = "crate::units::Size";
                  if (b)
                    rustDb.WriteLine("pub size: " + typeName + ",");
                  else
                    rustDb.WriteLine("pub size: Option<" + typeName + ">,");
                  continue;
                case "low_temp_warning":
                case "low_temp_fatal":
                case "high_temp_warning":
                case "high_temp_fatal":
                  typeName = "crate::units::Temperature";
                  break;
                case "decor":
                  typeName = "crate::units::Decor";
                  break;
                default:
                  if (type == typeof(string))
                    ...
                    typeName = "&'static [Option<f32>]";
                  else if (type == typeof((float, float)?))
                    typeName = "(f32, f32)";
                  else if (type == typeof(string[]))
                    typeName = "&'static [&'static str]";
                  else if (type == typeof(List<(string, List<string>, float)>))
                    continue;
                  else if (type == typeof(List<string>))
                    typeName = "&'static [&'static str]";
                  else
                  {
                    rustDb.WriteLine("// " + name + " " + type.FullName);
                    continue;
                  }
                  break;
              }
              if (b)
                rustDb.WriteLine("pub " + name + ": " + typeName + ",");
              else
                rustDb.WriteLine("pub " + name + ": Option<" + typeName + ">,");
            }
            rustDb.WriteLine("}");
            ...
            {
              rustDb.WriteLine("pub static " + entityName + ":Entity = Entity::new(");
              rustDb.WriteLine("crate::lang::Game::" + entityName + ",");
              rustDb.WriteLine("\"" + entityName + "\",");
              rustDb.WriteLine(entity.menu_order.ToString("0.000") + ",");
              rustDb.WriteLine("Params::" + category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1) + "(" + category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1) + "{");
              foreach (var ((name, type), b) in list)
              {
                var value = entity.GetType().GetField(name).GetValue(entity);
                if (type == typeof(List<(string, List<string>, float)>))
                  continue;
                else if (name == "menu_order" || name == "width" || name == "decor_radius")
                  continue;
                else if (value is null)
                {
                  if (name == "height")
                    rustDb.WriteLine("size: None,");
                  else
                    rustDb.WriteLine(name + ": None,");
                }
                else if (name == "molar_mass")
                {
                  if (b)
                    rustDb.WriteLine(name + ": crate::units::GramPerMole(" + (value as float?).Value.ToString("0.000") + "),");
                  else
                    rustDb.WriteLine(name + ": Some(crate::units::GramPerMole(" + (value as float?).Value.ToString("0.000") + ")),");
                }
                else if (name == "shc")
                {
                  if (b)
                    rustDb.WriteLine(name + ": crate::units::DtuPerGramPerDegree(" + (value as float?).Value.ToString("0.000") + "),");
                  else
                    rustDb.WriteLine(name + ": Some(crate::units::DtuPerGramPerDegree(" + (value as float?).Value.ToString("0.000") + ")),");
                }
                else if (name == "thermal_conductivity")
                {
                  if (b)
                    rustDb.WriteLine(name + ": crate::units::DtuPerMetreSecondPerDegree(" + (value as float?).Value.ToString("0.000") + "),");
                  else
                    rustDb.WriteLine(name + ": Some(crate::units::DtuPerMetreSecondPerDegree(" + (value as float?).Value.ToString("0.000") + ")),");
                }
                else if (name == "radiation_absorption_factor"
                || name == "flood_threshold"
                || name == "light_absorption")
                {
                  if (b)
                    rustDb.WriteLine(name + ": crate::units::Percent(" + (value as float?).Value.ToString("0.000") + "),");
                  else
                    rustDb.WriteLine(name + ": Some(crate::units::Percent(" + (value as float?).Value.ToString("0.000") + ")),");
                }
                else if (name == "mass_per_unit"
                || name == "chunk_mass"
                || name == "mass"
                || name == "default_mass")
                {
                  if (b)
                    rustDb.WriteLine(name + ": crate::units::Kg(" + (value as float?).Value.ToString("0.000") + "),");
                  else
                    rustDb.WriteLine(name + ": Some(crate::units::Kg(" + (value as float?).Value.ToString("0.000") + ")),");
                }
                else if (name == "height")
                {
                  if (b)
                    rustDb.WriteLine("size: crate::units::Size {width: " + entity.width + ", height: " + value + "},");
                  else
                    rustDb.WriteLine("size: Some(crate::units::Size {width: " + entity.width + ", height: " + value + "}),");
                }
                else if (name == "decor")
                {
                  if (b)
                    rustDb.WriteLine("decor: crate::units::Decor {value: " + entity.decor + ", radius: " + entity.decor_radius + "},");
                  else
                    rustDb.WriteLine("decor: Some(crate::units::Decor {value: " + entity.decor + ", radius: " + entity.decor_radius + "}),");
                }
                else if (name == "low_temp_warning"
                || name == "low_temp_fatal"
                || name == "high_temp_warning"
                || name == "high_temp_fatal")
                {
                  if (b)
                    rustDb.WriteLine(name + ": crate::units::Temperature(" + (value as float?).Value.ToString("0.000") + "),");
                  else
                    rustDb.WriteLine(name + ": Some(crate::units::Temperature(" + (value as float?).Value.ToString("0.000") + ")),");
                }
                else if (name == "tag")
                {
                  rustTag.WriteLine("  " + value + ",");
                  rustGame.WriteLine("  " + value + " => r#\"" + translations[value as string].name + "\"#,");
                  rustDesc.WriteLine("  " + value + " => r#\"" + translations[value as string].desc + "\"#,");
                }
                else if (name == "primary_element")
                {
                  if (b)
                    rustDb.WriteLine(name + ": &" + value + ",");
                  else
                    rustDb.WriteLine(name + ": Some(&" + value + "),");
                }
                else if (name == "material_category")
                {
                  if (b)
                    rustDb.WriteLine(name + ": &[");
                  else
                    rustDb.WriteLine(name + ": Some(&[");
                  foreach (var tag in value as string[])
                  {
                    if (ElementLoader.elements.Find(e => e.tag.Name == tag) is not null)
                      rustDb.WriteLine("MaterialCategory::Solid(&" + tag + "),");
                    else
                    {
                      Debug.Log("hmm  " + entityName);
                      rustDb.WriteLine("MaterialCategory::Tag(Tag::" + tag + "),");
                    }
                  }
                  if (b)
                    rustDb.WriteLine("],");
                ...
                else if (name == "tags")
                {
                  if (b)
                    ...
                    rustDb.WriteLine(name + ": Some(&[");
                  foreach (var tag in value as HashSet<string>)
                  {
                    tagList.Add(tag);
                    rustDb.WriteLine("Tag::" + tag + ",");
                  }
                  if (b)
                    rustDb.WriteLine("],");
                  else
                ...
                {
                  if (b)
                    rustDb.WriteLine(name + ": Mode::" + value + ",");
                  else
                    rustDb.WriteLine(name + ": Some(Mode::" + value + "),");
                  modeList.Add(value as string);
                }
                else if (name == "rotations")
                {
                  if (b)
                    rustDb.WriteLine(name + ": Rotation::" + value + ",");
                  else
                    rustDb.WriteLine(name + ": Some(Rotation::" + value + "),");
                  rotationList.Add(value as string);
                }
                else if (name == "location_rule")
                {
                  if (b)
                    rustDb.WriteLine(name + ": LocationRule::" + value + ",");
                  else
                    rustDb.WriteLine(name + ": Some(LocationRule::" + value + "),");
                  locationList.Add(value as string);
                }
                else if (name == "trap_critters")
                {
                  rustDb.WriteLine(name + ": Some(&[");
                  foreach (var tag in value as List<string>)
                  {
                ...
                else if (name == "dlc")
                {
                  ...
                  foreach (var tag in value as string[])
                  {
                    ...
                    if (tag == "Space Out")
                      d = "SpaceOut";
                    rustDb.WriteLine("crate::db::Dlc::" + d + ",");
                  }
                  if (b)
                    rustDb.WriteLine("],");
                  else
                ...
                {
                  if (b)
                    rustDb.WriteLine(name + ": \"" + (value as string) + "\",");
                  else
                    rustDb.WriteLine(name + ": Some(\"" + (value as string) + "\"),");
                }
                else if (type == typeof(float) || type == typeof(float?))
                {
                  if (b)
                    rustDb.WriteLine(name + ": " + (value as float?).Value.ToString("0.000") + ",");
                  else
                    rustDb.WriteLine(name + ": Some(" + (value as float?).Value.ToString("0.000") + "),");
                }
                else if (type == typeof(int) || type == typeof(int?))
                {
                  if (b)
                    rustDb.WriteLine(name + ": " + (value as int?) + ",");
                  else
                    rustDb.WriteLine(name + ": Some(" + (value as int?) + "),");
                }
                else if (type == typeof(bool) || type == typeof(bool?))
                {
                  if (b)
                    rustDb.WriteLine(name + ": " + ((value as bool?).Value ? "true," : "false,"));
                  else
                    rustDb.WriteLine(name + ": Some(" + ((value as bool?).Value ? "true)," : "false),"));
                }
                else if (type == typeof(List<float?>))
                {
                  if (b)
                    ...
                    rustDb.WriteLine(name + ": Some(&[");
                  foreach (var tag in value as List<float?>)
                  {
                    if (tag is null)
                      rustDb.WriteLine("None,");
                    else
                      rustDb.WriteLine("Some(" + tag.Value.ToString("0.000") + "),");
                  }
                  if (b)
                    rustDb.WriteLine("],");
                  else
                  ...
                  rustDb.WriteLine(name + ": Some(("
                  + (value as (float, float)?).Value.Item1.ToString("0.000")
                  + ", "
                  + (value as (float, float)?).Value.Item2.ToString("0.000")
                  + ")),");
                else if (type == typeof(string[]))
                {
                  if (b)
        ...
        rustTag.WriteLine("}");
        rustGame.WriteLine("  #[allow(unreachable_patterns)]");
        rustGame.WriteLine("  _ => \"Not translated\"");
        rustGame.WriteLine("}}");
        rustDesc.WriteLine("  #[allow(unreachable_patterns)]");
        rustDesc.WriteLine("  _ => \"Not translated\"");
        rustDesc.WriteLine("}}");

        rustDb.WriteLine("pub enum Tag {");
        foreach (var tag in tagList)
          rustDb.WriteLine("  " + tag + ",");
        rustDb.WriteLine("}");

        rustDb.WriteLine("pub enum Link {");
        foreach (var tag in linksList)
          rustDb.WriteLine("  " + tag + ",");
        rustDb.WriteLine("}");

        rustDb.WriteLine("pub enum Mode {");
        foreach (var tag in modeList)
          rustDb.WriteLine("  " + tag + ",");
        rustDb.WriteLine("}");

        rustDb.WriteLine("pub enum LocationRule {");
        foreach (var tag in locationList)
          rustDb.WriteLine("  " + tag + ",");
        rustDb.WriteLine("}");

        rustDb.WriteLine("pub enum Rotation {");
        foreach (var tag in rotationList)
          rustDb.WriteLine("  " + tag + ",");
        rustDb.WriteLine("}");
...
"""
        ...
        rustDb.WriteLine("pub enum Params {");
        using var rustCategory = new StreamWriter(Path.Combine(dir, "src", "lang", "temp", "category.rs"));
        rustCategory.WriteLine("""
// Autogenerated
#![allow(non_upper_case_globals)]
#![allow(dead_code)]
...
""");
        ...
        foreach (var (category, subCategory) in dico)
        {
          rustCategory.WriteLine("pub static " + category + ": Category = Category {");
          rustCategory.WriteLine("  ui: crate::lang::Text::Ui(crate::lang::Ui::Category" + category[0].ToString().ToUpper() + category.Substring(1) + "),");
          rustCategory.WriteLine("  sub_categories: &[");
          foreach (var (sub, _) in subCategory)
          {
            rustCategory.WriteLine("    &" + category + "_" + sub.ToLower() + ",");
          }
          rustCategory.WriteLine("]};");
          rustUi.WriteLine("    Category" + category[0].ToString().ToUpper() + category.Substring(1) + ",");
        }
        ...
        foreach (var (category, subCategory) in dico)
        {
          foreach (var (sub, entities) in subCategory)
          {
            rustCategory.WriteLine("pub static " + category + "_" + sub.ToLower() + ": SubCategory = SubCategory {");
            rustCategory.WriteLine("  ui: crate::lang::Text::Ui(crate::lang::Ui::Category" + category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1) + "),");
            rustCategory.WriteLine("  items: &[");
            foreach (var (entity, _) in entities)
              rustCategory.WriteLine("    &crate::db::" + entity + ",");
            rustCategory.WriteLine("]};");
            rustUi.WriteLine("    Category" + category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1) + ",");
            rustDb.WriteLine("  " + category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1) + "(" + category[0].ToString().ToUpper() + category.Substring(1) + sub[0].ToString().ToUpper() + sub.Substring(1) + "),");
          }
        }
        rustUi.WriteLine("];");
        rustDb.WriteLine("}");
...
""");
        Adult.ToRustClass(rustDb);
        Birth.ToRustClass(rustDb);
        Recipe.Building.ToRustClass(rustDb);
        Complex.ToRustClass(rustDb);
        Recipe.Death.ToRustClass(rustDb);
        Egg.ToRustClass(rustDb);
        Fertilization.ToRustClass(rustDb);
        Harvest.ToRustClass(rustDb);
        Irrigation.ToRustClass(rustDb);
        Phase.ToRustClass(rustDb);
        Rotting.ToRustClass(rustDb);
        Seeds.ToRustClass(rustDb);
        SublimateChunk.ToRustClass(rustDb);
        SublimateLiquid.ToRustClass(rustDb);
        SublimateSolid.ToRustClass(rustDb);
        rustDb.WriteLine("""
pub enum Recipe {
Adult(Adult),
Birth(Birth),
...
""");
      ...
      public static void ALaMano()
      {
        ...
        db["GravitasPedestal"].menu = Category.Building.Gravitas;
        db["TeleportalPad"].menu = Category.Building.Gravitas;
        db["Headquarters"].menu = Category.Building.Gravitas;
        db["RocketEnvelopeWindowTile"].menu = Category.Building.Gravitas;
        db["RocketInteriorGasInputPort"].menu = Category.Building.Gravitas;
        db["RocketInteriorGasOutputPort"].menu = Category.Building.Gravitas;
        db["RocketInteriorLiquidInputPort"].menu = Category.Building.Gravitas;
        db["RocketInteriorLiquidOutputPort"].menu = Category.Building.Gravitas;
      }
        ...
        foreach (var tag in tags)
        {
          ...
          switch (tag)
          {
            ...
            case "NotRocketInteriorBuilding":
              entity.not_in_rocket = true;
              break;
            case "RocketInteriorBuilding":
              entity.only_in_rocket = true;
              break;
            case "LuxuryBedType":
              break;
